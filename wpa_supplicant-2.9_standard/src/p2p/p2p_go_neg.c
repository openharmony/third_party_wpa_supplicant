...
int p2p_connect_send(struct p2p_data *p2p, struct p2p_device *dev)
{
	...
	if (req == NULL)
		return -1;
	p2p_err(p2p, "Sending GO Negotiation Request");
			    ...
			    const u8 *data, size_t len, int rx_freq)
{
	...
	p2p_err(p2p, "Received GO Negotiation Request from " MACSTR_SEC "(freq=%d)",
...
fail:
	...
	if (resp == NULL)
		return;
	p2p_err(p2p, "Sending GO Negotiation Response");
			     ...
			     const u8 *data, size_t len, int rx_freq)
{
	...
	if (*msg.status) {
		p2p_err(p2p, "GO Negotiation rejected: status %d", *msg.status);
			     ...
			     const u8 *data, size_t len)
{
	...
	p2p_err(p2p, "Received GO Negotiation Confirm from " MACSTR_SEC,
		MAC2STR_SEC(sa));
	dev = p2p_get_device(p2p, sa);
	if (dev == NULL || dev->wps_method == WPS_NOT_READY ||
	    dev != p2p->go_neg_peer) {
