					  ...
					  size_t key_data_len)
{
	...
	if (sm->mlo.valid_links)
		mlo_kde_len = wpa_mlo_link_kde_len(sm) + RSN_SELECTOR_LEN + ETH_ALEN + 2;
#endif
	...
	if (sm->mlo.valid_links) {
		...
		pos = kde + kde_len;
		pos = rsn_add_kde(pos, RSN_KEY_DATA_MAC_ADDR, sm->own_addr, ETH_ALEN);
	...
	if (wpa_supplicant_send_2_of_4(sm,
#ifdef CONFIG_MLD_PATCH
	                   wpa_sm_get_auth_addr(sm),
#else
                       sm->bssid,
					    ...
					    size_t link_kde_len)
{
	...
	if (os_memcmp(sm->mlo.links[link_id].bssid,
		&link_kde[RSN_MLO_LINK_KDE_LINK_MAC_INDEX], ETH_ALEN) != 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
...
int fils_process_assoc_resp(struct wpa_sm *sm, const u8 *resp, size_t len)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "FILS: Set TK to driver",
			sm->ptk.tk, keylen);
#ifdef CONFIG_MLD_PATCH
	if (wpa_sm_set_key(sm, -1, alg, wpa_sm_get_auth_addr(sm), 0, 1,  null_rsc, rsclen,
#else
