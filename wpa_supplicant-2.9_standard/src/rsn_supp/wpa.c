...
static int wpa_supplicant_activate_ptk(struct wpa_sm *sm)
{
	...
	wpa_printf(MSG_DEBUG, "WPA: Activate PTK (idx=%d auth_addr=" MACSTR_SEC ")",
		sm->keyidx_active, MAC2STR_SEC(wpa_sm_get_auth_addr(sm)));
	...
	if (wpa_sm_set_key(sm, -1, 0, wpa_sm_get_auth_addr(sm),
		...
		wpa_printf(MSG_WARNING, "WPA: Failed to activate PTK for TX (idx=%d auth_addr="
			MACSTR_SEC ")", sm->keyidx_active, MAC2STR_SEC(wpa_sm_get_auth_addr(sm)));
		return -1;
	}
	return 0;
}
...
static int fils_ft_build_assoc_req_rsne(struct wpa_sm *sm, struct wpabuf *buf)
{
	...
	wpa_printf(MSG_DEBUG, "FILS+FT: R1KH-ID: " MACSTR_SEC,
		   MAC2STR_SEC(sm->r1kh_id));
...
int fils_process_assoc_resp(struct wpa_sm *sm, const u8 *resp, size_t len)
{
	...
	if (wpa_sm_set_key(sm, -1, alg, wpa_sm_get_auth_addr(sm), 0, 1,
		...
		wpa_printf(MSG_WARNING, "FILS: Failed to set PTK to the driver (alg=%d keylen=%d auth_addr="
			MACSTR_SEC ")",
