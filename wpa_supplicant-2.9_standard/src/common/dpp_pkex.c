				...
				bool v2)
{
	...
	if (!is_zero_ether_addr(dpp_pkex_own_mac_override)) {
		wpa_printf(MSG_INFO, "DPP: TESTING - own_mac override " MACSTR_SEC,
			   MAC2STR_SEC(dpp_pkex_own_mac_override));
					   ...
					   const u8 *buf, size_t len, bool v2)
{
	...
	if (!is_zero_ether_addr(dpp_pkex_peer_mac_override)) {
		wpa_printf(MSG_INFO, "DPP: TESTING - peer_mac override " MACSTR_SEC,
			   MAC2STR_SEC(dpp_pkex_peer_mac_override));
