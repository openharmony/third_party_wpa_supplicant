				   ...
				   const char *driver_params)
{
	...
	wpa_printf(MSG_EXCESSIVE, "nl80211: interface %s in phy %s",
					...
					int link_id)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: send_mlme - da=" MACSTR_SEC " sa=" MACSTR_SEC
		   " bssid=" MACSTR_SEC
...
#ifdef CONFIG_KERNEL_VERSION_ADAPTER
				      ...
				      struct hostapd_sta_add_params *params)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: %s STA " MACSTR_SEC,
		   cmd_string, MAC2STR_SEC(params->addr));
...
static int wpa_driver_nl80211_set_supp_port(void *priv, int authorized)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Set supplicant port %sauthorized for "
		   MACSTR_SEC, authorized ? "" : "un", MAC2STR_SEC(connected_addr));
...
static int nl80211_send_pasn_resp(void *priv, struct pasn_auth *params)
{
	...
	for (i = 0; i < params->num_peers; i++) {
		...
		wpa_printf(MSG_DEBUG,
			   "nl80211: Own address[%u]: " MACSTR_SEC
			   " Peer address[%u]: " MACSTR_SEC " Status: %s",
			   i, MAC2STR_SEC(params->peer[i].own_addr), i,
			   MAC2STR_SEC(params->peer[i].peer_addr),
...
#endif /* CONFIG_IEEE80211AX */
#if defined(ANDROID_P2P) || defined(CONFIG_DRIVER_NL80211_HISI)
