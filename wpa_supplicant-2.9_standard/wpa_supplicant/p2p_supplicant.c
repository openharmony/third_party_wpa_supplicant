...
static int wpas_p2p_num_unused_channels(struct wpa_supplicant *wpa_s)
{
	...
	wpa_msg_only_for_cb(wpa_s, MSG_DEBUG, "P2P: num_unused_channels: %d", unused);
	wpa_printf(MSG_DEBUG, "P2P: num_unused_channels: %d", unused);
	return unused;
}
			 ...
			 bool include_6ghz)
{
	...
	case P2P_SCAN_POSSIBLE_CHANNEL:
		if (p2p_onehop_add_possible_freqs(wpa_s->global->p2p, params) ==
			0)
			goto fail;
		break;
#endif
				 ...
				 enum p2p_group_removal_reason removal_reason)
{
	...
	if (wpa_s->cross_connect_in_use) {
		...
		wpa_printf(MSG_INFO, P2P_EVENT_CROSS_CONNECT_DISABLE "%s %s",
			wpa_s->ifname, wpa_s->cross_connect_uplink);
	}
				     ...
				     const u8 *ssid, size_t ssid_len)
{
	...
	if (bss == NULL) {
		wpa_printf(MSG_DEBUG, "P2P: Could not figure out whether "
			   "group is persistent - BSS " MACSTR_SEC " not found",
			   MAC2STR_SEC(bssid));
		return 0;
	}
	...
	if (p2p == NULL) {
		wpa_printf(MSG_DEBUG, "P2P: Could not figure out whether "
			   "group is persistent - BSS " MACSTR_SEC
			   " did not include P2P IE", MAC2STR_SEC(bssid));
	...
	if (addr) {
		...
		wpa_printf(MSG_DEBUG, "P2P: GO Device Address " MACSTR_SEC,
			   MAC2STR_SEC(addr));
	} else
		os_memset(go_dev_addr, 0, ETH_ALEN);
	wpabuf_free(p2p);

	wpa_printf(MSG_DEBUG, "P2P: BSS " MACSTR_SEC " group_capab=0x%x "
		   "go_dev_addr=" MACSTR_SEC,
		   MAC2STR_SEC(bssid), group_capab, MAC2STR_SEC(go_dev_addr));
				   ...
				   const char *extra)
{
	...
	wpa_printf(MSG_INFO, P2P_EVENT_GROUP_STARTED
		   "%s %s ssid=\"%s\" freq=%d go_dev_addr=" MACSTR_SEC "%s%s",
...
static void wpas_p2p_free_send_action_work(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_DEBUG,
		   ...
		   MACSTR_SEC " src=" MACSTR_SEC " bssid=" MACSTR_SEC " wait_time=%u)",
		   wpa_s->p2p_send_action_work, awork->freq,
		   MAC2STR_SEC(awork->dst), MAC2STR_SEC(awork->src),
		   MAC2STR_SEC(awork->bssid), awork->wait_time);
					   ...
					   result)
{
	...
	if (result != OFFCHANNEL_SEND_ACTION_SUCCESS &&
		...
		wpa_printf(MSG_INFO, P2P_EVENT_FALLBACK_TO_GO_NEG
			       "reason=no-ACK-to-PD-Req");
				    ...
				    struct p2p_go_neg_results *res)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: Start WPS Enrollee for peer " MACSTR_SEC
		   " dev_addr " MACSTR_SEC " wps_method %d",
		   MAC2STR_SEC(res->peer_interface_addr),
		   MAC2STR_SEC(res->peer_device_addr), res->wps_method);
				  ...
				  struct wpa_ssid *ssid)
{
	...
	dl_list_for_each(psk, &persistent->psk_list, struct psk_list_entry,
		...
		wpa_msg_only_for_cb(wpa_s, MSG_DEBUG, "P2P: Add persistent group PSK entry for "
			MACSTR " psk=%d",
			MAC2STR(psk->addr), psk->p2p);
		wpa_printf(MSG_DEBUG, "P2P: Add persistent group PSK entry for "
			MACSTR_SEC " psk=%d",
			MAC2STR_SEC(psk->addr), psk->p2p);
	...
	if (ssid && ssid->mode == WPAS_MODE_P2P_GO) {
		...
		if (wpa_s->p2pdev->p2ps_method_config_any) {
			if (is_zero_ether_addr(wpa_s->p2pdev->p2ps_join_addr)) {
				...
				wpa_printf(MSG_DEBUG, "P2PS: Setting default PIN for " MACSTR_SEC,
					MAC2STR_SEC(wpa_s->p2pdev->p2ps_join_addr));
			      ...
			      int group_formation)
{
	...
	if (os_strlen(params->passphrase) > 0) {
		...
		if (ssid->passphrase == NULL) {
			...
			wpa_printf(MSG_ERROR, "P2P: Failed to copy passphrase for GO");
...
static int set_random_mac(struct wpa_supplicant *wpa_s)
{
	...
	int ret = memcpy_s(wpa_s->pending_interface_addr, sizeof(u8) * ETH_ALEN,
...
wpas_p2p_init_group_interface(struct wpa_supplicant *wpa_s, int go)
{
	...
	if (wpa_s->conf->p2p_interface_random_mac_addr) {
		...
		wpa_printf(MSG_DEBUG, "P2P: Using random MAC address " MACSTR_SEC
			   " for the group",
			   MAC2STR_SEC(wpa_s->pending_interface_addr));
	}
			   ...
			   int new_device)
{
	...
	if (info->p2ps_instance) {
		...
		while (len) {
			...
			wpa_msg_global(wpa_s, MSG_INFO,
				...
				info->vendor_elems ?
				" vendor_elems=1" : "");
			wpa_printf(MSG_INFO, P2P_EVENT_DEVICE_FOUND MACSTR_SEC
				" p2p_dev_addr=" MACSTR_SEC
				" pri_dev_type=%s name='%s'"
				" config_methods=0x%x"
				" dev_capab=0x%x"
				" group_capab=0x%x"
	...
	wpa_printf(MSG_INFO, P2P_EVENT_DEVICE_FOUND MACSTR_SEC
		...
		" pri_dev_type=%s name='%s' config_methods=0x%x "
		"dev_capab=0x%x group_capab=0x%x%s%s%s new=%d",
		MAC2STR_SEC(addr), MAC2STR_SEC(info->p2p_device_addr),
		wps_dev_type_bin2str(info->pri_dev_type, devtype,
		sizeof(devtype)),
		anonymize_common(info->device_name), info->config_methods,
		info->dev_capab, info->group_capab,
		wfd_dev_info_hex ? " wfd_dev_info=0x" : "",
		wfd_dev_info_hex ? wfd_dev_info_hex : "",
		info->vendor_elems ? " vendor_elems=1" : "",
...
static void wpas_start_listen_cb(struct wpa_radio_work *work, int deinit)
{
	...
	if (wpa_drv_remain_on_channel(wpa_s, lwork->freq, duration) < 0) {
		...
		/*
		* After the listen fails, the recovery status is IDLE,
		* so that the listen process can be entered again.
		*/
...
#ifdef CONFIG_LIBWPA_VENDOR
	struct P2pProvisionDiscoveryCompletedParam p2pProvisionDiscoveryCompletedParam;
	p2pProvisionDiscoveryCompletedParam.provDiscStatusCode = PROV_DISC_TYPE_ENTER_PIN;
	os_memcpy(p2pProvisionDiscoveryCompletedParam.p2pDeviceAddress, peer, ETH_ALEN);
...
#ifdef CONFIG_LIBWPA_VENDOR
		struct P2pProvisionDiscoveryCompletedParam p2pProvisionDiscoveryCompletedParam;
		p2pProvisionDiscoveryCompletedParam.provDiscStatusCode = PROV_DISC_TYPE_PBC_REQ;
		os_memcpy(p2pProvisionDiscoveryCompletedParam.p2pDeviceAddress, peer, ETH_ALEN);
		wpa_printf(MSG_INFO, "WPA_EVENT_PROVISION_DISCOVERY_COMPLETED " MACSTR_SEC,
			MAC2STR_SEC(p2pProvisionDiscoveryCompletedParam.p2pDeviceAddress));
		WpaEventReport(wpa_s->ifname, WPA_EVENT_PROVISION_DISCOVERY_COMPLETED,
...
#ifdef CONFIG_LIBWPA_VENDOR
		struct P2pProvisionDiscoveryCompletedParam p2pProvisionDiscoveryCompletedParam;
		p2pProvisionDiscoveryCompletedParam.provDiscStatusCode = PROV_DISC_TYPE_PBC_RESP;
		os_memcpy(p2pProvisionDiscoveryCompletedParam.p2pDeviceAddress, peer, ETH_ALEN);
		wpa_printf(MSG_INFO, "WPA_EVENT_PROVISION_DISCOVERY_COMPLETED " MACSTR_SEC,
			MAC2STR_SEC(p2pProvisionDiscoveryCompletedParam.p2pDeviceAddress));
		WpaEventReport(wpa_s->ifname, WPA_EVENT_PROVISION_DISCOVERY_COMPLETED,
				...
				const char *deferred_session_resp)
{
	...
	if (adv_id && adv_mac && deferred_session_resp) {
		...
		wpa_printf(MSG_INFO, P2P_EVENT_PROV_DISC_FAILURE
			" p2p_dev_addr=" MACSTR_SEC " status=%d adv_id=%x"
			" deferred_session_resp='%s'",
			MAC2STR_SEC(peer), status, adv_id,
			deferred_session_resp);
...
#ifdef CONFIG_LIBWPA_VENDOR
	struct P2pProvisionDiscoveryCompletedParam p2pProvisionDiscoveryCompletedParam;
	p2pProvisionDiscoveryCompletedParam.provDiscStatusCode = PROV_DISC_TYPE_FAILURE;
				  ...
				  int dev_pw_id)
{
	...
	if (!persistent_group) {
		wpa_printf(MSG_DEBUG, "P2P: Invitation from " MACSTR_SEC
			   " to join an active group (SSID: %s)",
			   MAC2STR_SEC(sa), anonymize_ssid(wpa_ssid_txt(ssid, ssid_len)));
				     ...
				     int op_freq)
{
	...
	if (status == P2P_SC_SUCCESS) {
		wpa_printf(MSG_DEBUG, "P2P: Invitation from peer " MACSTR_SEC
			   " was accepted; op_freq=%d MHz, SSID=%s",
			   MAC2STR_SEC(sa), op_freq, anonymize_ssid(wpa_ssid_txt(ssid, ssid_len)));
		if (s) {
			...
			if (go) {
				...
				wpa_printf(MSG_INFO, P2P_EVENT_INVITATION_ACCEPTED
					"sa=" MACSTR_SEC
					" persistent=%d freq=%d",
					...
					const u8 *peer, int inv)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: Remove peer " MACSTR_SEC " from persistent "
		   "group %d client list%s",
		   MAC2STR_SEC(peer), ssid->id,
				   ...
				   int peer_oper_freq)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: Persistent group invitation success - op_freq=%d MHz SSID=%s",
		   freq, anonymize_ssid(wpa_ssid_txt(ssid->ssid, ssid->ssid_len)));
				    ...
				    const u8 *group_ssid, size_t group_ssid_len)
{
	...
	if (prov_start) {
		...
		wpa_printf(MSG_INFO, P2P_EVENT_P2PS_PROVISION_START MACSTR_SEC
				" adv_id=%x conncap=%x"
				" adv_mac=" MACSTR_SEC
				" session=%x mac=" MACSTR_SEC
				" dev_passwd_id=%d%s",
				MAC2STR_SEC(dev), adv_id, conncap,
				MAC2STR_SEC(adv_mac),
				ses_id, MAC2STR_SEC(ses_mac),
				passwd_id, feat_cap_str);
		} else {
			wpa_msg_global(wpa_s, MSG_INFO,
				P2P_EVENT_P2PS_PROVISION_START MACSTR
	...
	if (status && status != P2P_SC_SUCCESS_DEFERRED) {
		...
		wpa_printf(MSG_INFO, P2P_EVENT_P2PS_PROVISION_DONE MACSTR_SEC
			       " status=%d"
			       " adv_id=%x adv_mac=" MACSTR_SEC
			       " session=%x mac=" MACSTR_SEC "%s",
			       MAC2STR_SEC(dev), status,
			       adv_id, MAC2STR_SEC(adv_mac),
			       ses_id, MAC2STR_SEC(ses_mac), feat_cap_str);
		return;
	}
	...
	if (s) {
		...
		wpa_printf(MSG_INFO, P2P_EVENT_P2PS_PROVISION_DONE MACSTR_SEC
			       ...
			       MAC2STR_SEC(dev), status,
			       adv_id, MAC2STR_SEC(adv_mac),
			       ses_id, MAC2STR_SEC(ses_mac), s->id, feat_cap_str);
		return;
	}
	...
	if (conncap == P2PS_SETUP_GROUP_OWNER) {
		...
		if (!go_wpa_s) {
			...
			if (is_zero_ether_addr(grp_mac)) {
				...
				wpa_msg_only_for_cb(go_wpa_s, MSG_DEBUG,
					"P2P: Setting PIN-1 for " MACSTR,
					MAC2STR(grp_mac));
				wpa_printf(MSG_DEBUG, "P2P: Setting PIN-1 for " MACSTR_SEC,
					MAC2STR_SEC(grp_mac));
		...
		wpa_printf(MSG_INFO, P2P_EVENT_P2PS_PROVISION_DONE MACSTR_SEC
			...
			" session=%x mac=" MACSTR_SEC
			" dev_passwd_id=%d go=%s%s",
			MAC2STR_SEC(dev), status, conncap,
			adv_id, MAC2STR_SEC(adv_mac),
			ses_id, MAC2STR_SEC(ses_mac),
			passwd_id, go_ifname, feat_cap_str);
		return;
	...
	if (conncap == P2PS_SETUP_CLIENT) {
		...
		wpa_printf(MSG_INFO, P2P_EVENT_P2PS_PROVISION_DONE MACSTR_SEC
			...
			" adv_id=%x adv_mac=" MACSTR_SEC
			" session=%x mac=" MACSTR_SEC
			" dev_passwd_id=%d join=" MACSTR_SEC "%s%s%s",
			MAC2STR_SEC(dev), status, conncap,
			adv_id, MAC2STR_SEC(adv_mac),
			ses_id, MAC2STR_SEC(ses_mac),
			passwd_id, MAC2STR_SEC(grp_mac), feat_cap_str,
			group_ssid ? " group_ssid=" : "", ssid_hex);
	} else {
		wpa_msg_global(wpa_s, MSG_INFO,
			P2P_EVENT_P2PS_PROVISION_DONE MACSTR
...
static void wpas_p2p_check_join_scan_limit(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->p2p_join_scan_count > P2P_MAX_JOIN_SCAN_ATTEMPTS) {
		wpa_printf(MSG_DEBUG, "P2P: Failed to find GO " MACSTR_SEC
			   " for join operationg - stop join attempt",
			   MAC2STR_SEC(wpa_s->pending_join_iface_addr));
		eloop_cancel_timeout(wpas_p2p_join_scan, wpa_s, NULL);
		if (wpa_s->p2p_auto_pd) {
			...
			wpa_printf(MSG_INFO, P2P_EVENT_PROV_DISC_FAILURE
				" p2p_dev_addr=" MACSTR_SEC " status=N/A",
				MAC2STR_SEC(wpa_s->pending_join_dev_addr));
			return;
		}
		if (wpa_s->p2p_fallback_to_go_neg) {
			wpa_dbg(wpa_s, MSG_DEBUG,
				   ...
				   struct wpa_scan_results *scan_res)
{
	...
	if (wpa_s->p2p_auto_pd) {
		...
		wpa_printf(MSG_DEBUG, "P2P: Auto PD with " MACSTR_SEC " join=%d",
			   MAC2STR_SEC(wpa_s->pending_join_dev_addr), join);
	...
	if (wpa_s->p2p_auto_join) {
		...
		if (!join) {
			...
			wpa_printf(MSG_INFO, P2P_EVENT_FALLBACK_TO_GO_NEG_ENABLED);
	...
	if (freq < 0 &&
		...
		wpa_printf(MSG_DEBUG, "P2P: Overwrite pending interface "
			   "address for join from " MACSTR_SEC " to " MACSTR_SEC
			   " based on newly discovered P2P peer entry",
			   MAC2STR_SEC(wpa_s->pending_join_iface_addr),
			   MAC2STR_SEC(iface_addr));
	...
	if (wpa_s->p2p_join_ssid_len) {
		wpa_printf(MSG_DEBUG, "P2P: Trying to find target GO BSS entry based on BSSID "
			   MACSTR_SEC " and SSID %s",
			   MAC2STR_SEC(wpa_s->pending_join_iface_addr),
			   anonymize_ssid(wpa_ssid_txt(wpa_s->p2p_join_ssid,
		...
		wpa_printf(MSG_DEBUG, "P2P: Trying to find target GO BSS entry based on BSSID "
			   MACSTR_SEC, MAC2STR_SEC(wpa_s->pending_join_iface_addr));
	...
	if (bss) {
		...
		if (p2p_parse_dev_addr(wpa_bss_ie_ptr(bss), bss->ie_len,
			...
			wpa_printf(MSG_DEBUG,
				   "P2P: Update target GO device address based on BSS entry: " MACSTR_SEC " (was " MACSTR_SEC ")",
	...
	if (freq > 0) {
		...
		wpa_printf(MSG_DEBUG, "P2P: Send Provision Discovery Request "
			   "prior to joining an existing group (GO " MACSTR_SEC
			   " freq=%u MHz)",
			   MAC2STR_SEC(wpa_s->pending_join_dev_addr), freq);
		...
		if ((p2p_get_provisioning_info(wpa_s->global->p2p,
			...
			wpa_printf(MSG_DEBUG, "P2P: Provision discovery "
				   "with " MACSTR_SEC " already done - proceed to "
				   "join",
				   MAC2STR_SEC(wpa_s->pending_join_dev_addr));
			wpa_s->pending_pd_before_join = 0;
			goto start;
	...
	if (join || auto_join) {
		...
		if (auth) {
			wpa_printf(MSG_DEBUG, "P2P: Authorize invitation to "
				   "connect a running group from " MACSTR_SEC,
				   ...
				   unsigned int freq, unsigned int duration)
{
	if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL)
		return;
#if defined(CONFIG_OPEN_HARMONY_PATCH) && defined(CONFIG_MIRACAST_SINK_OPT)
	miracast_sink_log("P2P: remain-on-channel callback (off_channel_freq=%u pending_listen_freq=%d roc_waiting_drv_freq=%d freq=%u duration=%u)",
...
{
#if defined(CONFIG_OPEN_HARMONY_PATCH) && defined(CONFIG_MIRACAST_SINK_OPT)
	miracast_sink_log("P2P: Cancel remain-on-channel callback "
			 ...
			 int go)
{
	...
	if (wpas_p2p_add_group_interface(wpa_s, go ? WPA_IF_P2P_GO :
		...
		wpa_printf(MSG_ERROR, "P2P: Failed to add group interface");
		return NULL;
	}
	...
	if (group_wpa_s == NULL) {
		...
		wpa_printf(MSG_ERROR, "P2P: Failed to initialize group interface");
		wpas_p2p_remove_pending_group_interface(wpa_s);
		return NULL;
	}
...
#ifdef CONFIG_OPEN_HARMONY_MIRACAST_MAC
	wpa_s->p2p_business = saved_business;
#endif
...
static void wpas_p2p_enable_cross_connect(struct wpa_supplicant *uplink)
{
	...
	for (iface = uplink->global->ifaces; iface; iface = iface->next) {
		...
		wpa_printf(MSG_INFO, P2P_EVENT_CROSS_CONNECT_ENABLE "%s %s",
			iface->ifname, iface->cross_connect_uplink);
	}
}
	...
	dl_list_for_each_safe(psk, tmp, &s->psk_list, struct psk_list_entry,
		...
		if ((iface_addr && !psk->p2p &&
			...
			wpa_msg_only_for_cb(wpa_s, MSG_DEBUG,
				...
				MACSTR " p2p=%u",
				MAC2STR(psk->addr), psk->p2p);
			wpa_printf(MSG_DEBUG, "P2P: Remove persistent group PSK list entry for "
				MACSTR_SEC " p2p=%u",
				MAC2STR_SEC(psk->addr), psk->p2p);
			 ...
			 const u8 *psk, size_t psk_len)
{
	...
	if (p2p_dev_addr) {
		wpa_msg_only_for_cb(wpa_s, MSG_DEBUG, "P2P: New PSK for addr=" MACSTR
			" p2p_dev_addr=" MACSTR,
			MAC2STR(mac_addr), MAC2STR(p2p_dev_addr));
		wpa_printf(MSG_DEBUG, "P2P: New PSK for addr=" MACSTR_SEC
			" p2p_dev_addr=" MACSTR_SEC,
	...
	if (dl_list_len(&persistent->psk_list) > P2P_MAX_STORED_CLIENTS &&
		...
		wpa_msg_only_for_cb(wpa_s, MSG_DEBUG, "P2P: Remove oldest PSK entry for "
			MACSTR " (p2p=%u) to make room for a new one",
			MAC2STR(last->addr), last->p2p);
		wpa_printf(MSG_DEBUG, "P2P: Remove oldest PSK entry for "
			MACSTR_SEC " (p2p=%u) to make room for a new one",
			MAC2STR_SEC(last->addr), last->p2p);
	...
	wpas_p2p_remove_psk_entry(wpa_s->p2pdev, persistent,
				  ...
				  p2p_dev_addr == NULL);
	if (p2p_dev_addr) {
		wpa_msg_only_for_cb(wpa_s, MSG_DEBUG, "P2P: Add new PSK for p2p_dev_addr="
			MACSTR, MAC2STR(p2p_dev_addr));
		wpa_printf(MSG_DEBUG, "P2P: Add new PSK for p2p_dev_addr="
			MACSTR_SEC, MAC2STR_SEC(p2p_dev_addr));
	} else {
		wpa_msg_only_for_cb(wpa_s, MSG_DEBUG, "P2P: Add new PSK for addr=" MACSTR,
			MAC2STR(mac_addr));
		wpa_printf(MSG_DEBUG, "P2P: Add new PSK for addr=" MACSTR_SEC,
			MAC2STR_SEC(mac_addr));
	}
				      ...
				      const u8 *peer, int iface_addr)
{
	...
	while (psk) {
		if ((iface_addr && ether_addr_equal(peer, psk->addr)) ||
			...
			wpa_msg_only_for_cb(wpa_s, MSG_DEBUG, "P2P: Remove operating group PSK entry for "
				MACSTR " iface_addr=%d",
				MAC2STR(peer), iface_addr);
			wpa_printf(MSG_DEBUG, "P2P: Remove operating group PSK entry for "
				MACSTR_SEC " iface_addr=%d",
				MAC2STR_SEC(peer), iface_addr);
			if (prev)
				prev->next = psk->next;
			else
	...
	if (sta) {
		wpa_msg_only_for_cb(wpa_s, MSG_DEBUG, "P2P: Disconnect peer " MACSTR
			" (iface_addr=%d) from group",
			MAC2STR(peer), iface_addr);
		wpa_printf(MSG_DEBUG, "P2P: Disconnect peer " MACSTR_SEC
			" (iface_addr=%d) from group",
			MAC2STR_SEC(peer), iface_addr);
...
int wpas_p2p_4way_hs_failed(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->p2p_last_4way_hs_fail &&
		...
		wpa_msg_only_for_cb(wpa_s, MSG_DEBUG, "P2P: Two 4-way handshake failures for a P2P group - go_dev_addr="
			MACSTR, MAC2STR(go_dev_addr));
		wpa_printf(MSG_DEBUG, "P2P: Two 4-way handshake failures for a P2P group - go_dev_addr="
			MACSTR_SEC, MAC2STR_SEC(go_dev_addr));
		...
		wpa_msg_global(wpa_s->p2pdev, MSG_INFO,
			       ...
			       persistent->id);
		wpa_printf(MSG_INFO, P2P_EVENT_PERSISTENT_PSK_FAIL "%d",
			persistent->id);
	disconnect:
		wpa_s->p2p_last_4way_hs_fail = NULL;
		/*
				  ...
				  struct p2p_nfc_params *params, int tag)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: Static handover - invite peer " MACSTR_SEC
		   " to join", MAC2STR_SEC(params->peer->p2p_device_addr));
					    ...
					    int sel, int tag, int forced_freq)
{
	...
	if (params.next_step == PEER_CLIENT) {
		if (!is_zero_ether_addr(params.go_dev_addr)) {
			wpa_msg_only_for_cb(wpa_s, MSG_INFO, P2P_EVENT_NFC_PEER_CLIENT
			...
			wpa_printf(MSG_INFO, P2P_EVENT_NFC_PEER_CLIENT
				"peer=" MACSTR_SEC " freq=%d go_dev_addr=" MACSTR_SEC
				" ssid=\"%s\"",
				MAC2STR_SEC(params.peer->p2p_device_addr),
				params.go_freq,
				MAC2STR_SEC(params.go_dev_addr),
				anonymize_ssid(wpa_ssid_txt(params.go_ssid,
				params.go_ssid_len)));
		} else {
...
static void wpas_p2p_move_go_no_csa(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_INFO, P2P_EVENT_REMOVE_AND_REFORM_GROUP);
					    ...
					    unsigned int num)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: GO move: invalid_freq=%u, policy_move=%u, flags=0x%X",
		invalid_freq, policy_move, flags);
	...
	if (!invalid_freq || policy_move) {
		...
		if (wpas_p2p_in_progress(wpa_s)) {
			...
			wpa_printf(MSG_DEBUG, "P2P: GO move: policy CS is not allowed - setting timeout to re-consider GO move");
