					 ...
					 int cipher, int akmp)
{
	...
	if (!entry) {
		wpa_printf(MSG_DEBUG, "PASN: peer " MACSTR_SEC
			   " not present in PTKSA cache", MAC2STR_SEC(peer_addr));
		return -1;
	}

	if (!ether_addr_equal(entry->own_addr, own_addr)) {
		wpa_printf(MSG_DEBUG,
			   "PASN: own addr " MACSTR_SEC " and PTKSA entry own addr "
			   MACSTR_SEC " differ",
			   MAC2STR_SEC(own_addr), MAC2STR_SEC(entry->own_addr));
		return -1;
	}

	wpa_printf(MSG_DEBUG, "PASN: " MACSTR_SEC " present in PTKSA cache",
		   MAC2STR_SEC(peer_addr));
			 ...
			 const u8 *comeback, size_t comeback_len)
{
	...
	wpa_printf(MSG_DEBUG, "PASN: Start: " MACSTR_SEC " akmp=0x%x, cipher=0x%x",
		   MAC2STR_SEC(peer_addr), akmp, cipher);
