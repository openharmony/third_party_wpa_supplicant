...
{
	hisi_ioctl_command_stru *ioctl_cmd = os_zalloc(sizeof(hisi_ioctl_command_stru));
	(void)memset_s(ioctl_cmd, sizeof(hisi_ioctl_command_stru), 0, sizeof(hisi_ioctl_command_stru));
	ioctl_cmd->cmd = cmd;
	ioctl_cmd->buf = (void *)buf;
	wpa_printf(MSG_ERROR, "hisi_ioctl_command_set cmd=%d, ioctl_cmd addr=%p", cmd, ioctl_cmd);
	return WifiWpaCmdSyncSend(ifname, ioctl_cmd);
}
...
int32 hisi_eapol_packet_receive(const int8 *ifname, hisi_rx_eapol_stru *rx_eapol)
{
    ...
    wpa_printf(MSG_ERROR, "hisi_eapol_packet_receive enter");
	int32 ret = hisi_ioctl_command_set(ifname, rx_eapol, HISI_IOCTL_RECEIVE_EAPOL);
    wpa_printf(MSG_ERROR, "hisi_eapol_packet_receive ret=%d", ret);
    return ret;
}
...
int32 hisi_ioctl_set_netdev(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, (void *)buf, HISI_IOCTL_SET_NETDEV);
}
